...
/************************** KeyGen **************************/
bswabe_prv_t*
bswabe_keygen( bswabe_pub_t* pub, bswabe_msk_t* msk, char** attributes, element_t u_k, char* out_file )
{
	bswabe_prv_t* prv;  	
	element_t g_r;
	element_t r;
	...
	/* compute */
	char *filename = 0;
	filename = g_strdup_printf("%s.txt", out_file);
	...
	//randomly generate a for user
	element_random(a);
	//randomly generate b for user
	element_random(b);	
	//randomly generate lambda for user
	element_random(lambda);		
	element_fprintf(ptr,"%B\n",a);
	element_fprintf(ptr,"%B\n",b);
	element_fprintf(ptr,"%B\n",lambda);
	fclose(ptr);
	...
	//element_set(p_0, (msk->rpt)->coef[0]); 					//p_0 = the secret
    	...
    	element_mul(p_0,lambda,a);

    	element_add(p_0,p_0,b);
...
void convert(bswabe_pub_t* pub, bswabe_cph_t *cph, char* u_k_str, element_t lambda_k, char* u_k_file)
{	
	element_t u_k;
	element_t sum;
	//GArray* s;
	...
	char *filename = (char*)malloc(sizeof(u_k_file)+1);
	int j=0;
	while(u_k_file[j]!='.')
	{
		filename[j] = u_k_file[j];
		j++;
	}
	filename[j] = '.';
	j++;
	...
	fl_size1 = ftell(ptr);
	rewind(ptr);
	buffer1 = (char*)calloc(fl_size1,sizeof(char));
	res1 = fread(buffer1,1,fl_size1,ptr);
	if(res1!=fl_size1)
	{
		printf("Reading error!");
	}
	char * strtok_res1 = NULL;
	strtok_res1 = strtok(buffer1,"\n");
	strtok_res1 = strtok(NULL,"\n");	
	element_set_str(sum,strtok_res1,10);
	strtok_res1 = strtok(NULL,"\n");
	element_set_str(lambda_k,strtok_res1,10);
	free(buffer1);
	fclose(ptr);	
...
/************************** Delegate **************************/
bswabe_del_prv_t*
bswabe_delegate( bswabe_pub_t* pub, bswabe_msk_t* msk, bswabe_prv_t* prv, char* u_k_c_str, char** attributes, char* u_k_file)
{
	int i = 0;
	...
	char * strtok_res1;
	strtok_res1 = strtok(buffer1,"\n");
	element_set_str(a,strtok_res1,10);
	strtok_res1 = strtok(NULL,"\n");	
	element_set_str(b,strtok_res1,10);
	strtok_res1 = strtok(NULL,"\n");
	element_set_str(lambda,strtok_res1,10);
	free(buffer1);
	fclose(ptr);
	...
	//Start of added by us
	element_t lab_sum;
	element_init_Zr(lab_sum, pub->p);
	element_mul(lab_sum,lambda,a);
	element_add(lab_sum,lab_sum,b);
	//End of added by us
	
	element_invert(p_0_inv, lab_sum); // 1/p(0) changed by us
	element_set_str(u_k_c, u_k_c_str, 10);

	element_set(p_c,a);  //Chnaged by us
